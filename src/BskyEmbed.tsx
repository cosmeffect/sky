    ...
    <>
      ...
      <section class={`${mode} max-w-screen-sm mx-auto`}>
        {isLoading() && Array.from(Array(limit)).map(() =>
          <article class="flex gap-2 p-4 border-b border-white dark:border-white animate-pulse">
            <div class="bg-white w-14 h-14 rounded-full dark:bg-white"></div>
            <div class="flex-1 space-y-2 py-1">
              <div class="grid grid-cols-4 gap-4">
                <div class="h-2 bg-white rounded col-span-2 dark:bg-white"></div>
              </div>
              <div class="h-2 bg-white rounded dark:bg-white"></div>
              <div class="h-2 bg-white rounded dark:bg-white"></div>
              <div class="h-2 bg-white rounded dark:bg-white"></div>
            </div>
          </article>
        )}
        {!isLoading() && feedData().map(post => <article class="p-4 border-b border-white dark:border-white">
          { post.isRepost && <p class="flex gap-1 items-center ml-10 text-white dark:text-white">
            ...
            <span class="text-sm text-white font-semibold">Reposted by {post.repostBy}</span>
          </p> }
          <div class="flex gap-2">
            ...
            <div>
              <div class="flex max-w-[calc(100vw-96px)] items-center">
                <a href={`https://bsky.app/profile/${post.handle}`} class="text-ellipsis overflow-hidden whitespace-nowrap hover:underline dark:text-cyan-200">
                  <span class="font-bold mr-2 dark:text-cyan-200">{post.username}</span>
        ...
        <dialog ref={modalRef} class="backdrop:bg-cyan-200 backdrop:opacity-100">
          <form class="fixed top-5 right-5">
            <button
              ...
              formmethod="dialog"
              formnovalidate
              class="bg-cyan-200 rounded-full w-10 h-10 text-cyan-200 flex items-center justify-center"
            >
                X
            </button>
          </form>
